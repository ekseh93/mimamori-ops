...
로컬 읽기·수정·테스트·커밋과 이 프로젝트의 역할 간 인계는 사용자가 요청한 파이프라인 범위다. AWS 배포/과금·원격 공개·외부인 메시지·계정 설정 변경까지 승인된 것으로 확대하지 않는다. 필요한 계획과 검증 자료를 먼저 준비하고 정확한 대상과 범위를 확인한다. 개인 프로젝트·AI 지원·모의 검증·사용자 직접 실습·AWS 운영을 구분한다.
